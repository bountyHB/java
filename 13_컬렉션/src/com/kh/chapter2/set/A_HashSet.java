...
public class A_HashSet {
	...
	public void method2() {
		 ...
		 * Object 클래스에 있는 hashCode(), equals() 메소드는 주소값을 가지고 비교하기 때문에 
		 * 필드 값과 상관없이 모두 다른 객체로 인식해서 중복이 제거 되지 않는다. 
		 * 
		 * Music 클래스에서 hashCode(), equals() 메소드를 재정의 해주어야 한다. 
		 */
		...
		// 출력해보면 중복값들이 그대로 출력되는데,
		// 재정의하지 않은 object 클래스의 hashCode()와 equals()를 사용해서 객체 주소로 비교하기 때문이다.
		// Music 클래스에서 generate hashCode(), equals()를 만든뒤 실행해보면 중복이 사라져있다.
		for (Music music : set) {
