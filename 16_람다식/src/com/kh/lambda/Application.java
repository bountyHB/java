...
public class Application {
	...
	public static void main(String[] args) {
		 ...
		 *
		 *	- 람다식을 사용하면 코드가 간결해지고, 
		 *    컬렉션의 요소를 필터링하거나 매핑해서 원하는 결과를 쉽게 가져올 수 있다.
		 * 
		 *   [표현법]
		 *  	([타입 매개변수명, ...]) -> {
		 *  		// 실행 코드 
		 ...
		 * 		- () -> {실행구문}; // 매개변수가 없다면 매개변수 자리에 반드시 빈 괄호를 사용해야 한다.
		 * 		- (a, b) -> {return a + b}; // 결과값을 리턴해야한다면 return문으로 결과값을 지정해야 한다.
		 * 		- (a, b) -> a + b; // return문 하나만 있으면 return문과 중괄호를 생략 가능 
		 * 
		 *  	자료형 변수 = (a, b) -> { ... } ;
		 *  	객체.메소드 (() -> {...}); 
		 */
...
//		new A_Lambda().method1();
		 ...
		 *    	두 개 이상의 추상메소드가 선언되지 않도록 컴파일러가 체크해 준다.
		 *  
		 */
...
//		new B_Functional().method3();
		 ...
		 *  - 자바 8부터 추가되거나 변경된 API는 표준 함수적 인터페이스들을 매개 타입으로 많이 사용한다.
...
//		new E_Predicate().method1();
		 ...
		 *  - 메소드 참조를 참조해서 매개 변수의 정보 및 리턴 타입을 알아내어,
